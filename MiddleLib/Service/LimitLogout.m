...
//    KeyTimeObj* keyTimeObj = [[KeyTimeObj alloc]init];
    ...
    for (long i=0; i<[arrayUsed count]; i++) {
        ...
        SMLog(@"No.(%d)- Conditon: LastLimit(%d), BUYPERCENT(%.2f), DESTDVVALUE(%.2f), duration(%d)  Result:totalAvgVal(%.2f),totalCount(%d), SelCount(%d), selAvg(%.2f) ",i, ele.daysAfterLastLimit,ele.buyPercent, ele.destDVValue,  ele.durationAfterBuy, ele.allAvgS2BDVValue ,ele.allTotalCount,ele.selTotalCount,ele.selAvgS2BDVValue );
    }
